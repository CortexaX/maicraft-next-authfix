...
// 决策记忆也会自动发送
this.state.memory.recordDecision('挖掘木头', { actionType: 'mine_by_type', params: { blockType: 'log' } }, 'success', '成功挖掘了3个木头');
```
...
```typescript
// 注册新的覆盖模板
overrideManager.registerTemplate('custom_template', '这是自定义的提示词内容', '自定义模板描述');
