...
async function runPythonAuth ({ pythonBin = 'python3', pythonAuthScript, username = '', password = '' }) {
  return await new Promise((resolve, reject) => {
    const child = spawn(pythonBin, [pythonAuthScript, username, password], { stdio: ['ignore', 'pipe', 'pipe'] })
    ...
    child.on('close', (code) => {
      if (code !== 0) {
        return reject(new Error(`python auth exited with code ${code}: ${stderr}`))
      }
      ...
      try {
        ...
        if (!parsed.token || !parsed.profile?.name || !parsed.profile?.id) {
          return reject(new Error('python auth missing required fields (token/profile.id/profile.name)'))
        }
        return resolve(parsed)
      } catch (e) {
        return reject(new Error(`python auth output parse failed: ${e.message}`))
      }
    })
  })
}
...
class MicrosoftAuthFlow {
  ...
  async getMinecraftJavaToken (options = {}) {
    const pythonAuthScript = options.pythonAuthScript || this.options.pythonAuthScript
    const pythonBin = options.pythonBin || this.options.pythonBin || 'python3'

    if (!pythonAuthScript) {
      throw new Error('Java auth now requires external pythonAuthScript. Legacy prismarine-auth Microsoft login flow has been removed.')
    }

    debug('[mc] using external python auth script:', pythonAuthScript)
    const response = await runPythonAuth({
      pythonBin,
      pythonAuthScript,
    ...
    if (!options.fetchEntitlements && !response.entitlements) response.entitlements = {}
    if (!options.fetchCertificates && !response.certificates) response.certificates = {}
